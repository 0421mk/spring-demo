...
public class UsrArticleController {
	...
	@ResponseBody
	private ResultData doDelete(HttpSession httpSession, int id) {
		boolean isLogined = false;
		int loginedMemberId = 0;
		...
		if (article.getMemberId() != loginedMemberId) {
			return ResultData.from("F-3", "작성자가 아닙니다.");
		}

		articleService.deleteArticle(id);

		return ResultData.from("S-1", Util.f("%d번 게시물을 삭제하였습니다.", id));
	}

	@RequestMapping("/usr/article/doModify")
	@ResponseBody
	private ResultData doModify(int id, String title, String body) {
		Article article = articleService.getArticle(id);
